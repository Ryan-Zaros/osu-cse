        ...
        switch (this.kind()) {
            case BLOCK: {
                for (int i = 0; i < this.lengthOfBlock(); i++) {
                    Statement s1 = this.removeFromBlock(i);
                    s1.prettyPrint(out, offset);
                    this.addToBlock(i, s1);
                }
                break;
            }
            case IF: {
                Statement s1 = new Statement1();
                Condition c = this.disassembleIf(s1);
            ...
            case WHILE: {
                Statement s1 = new Statement1();
                Condition c = this.disassembleWhile(s1);
                printSpaces(out, offset);
                out.println("WHILE " + toStringCondition(c));
                s1.prettyPrint(out, offset + Program.INDENT_SIZE);
