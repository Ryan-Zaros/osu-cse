...
public final class JCFExplorations {
    ...
    public static void giveRaise(components.map.Map<String, Integer> map,
        ...
        components.map.Map<String, Integer> tmp = map.newInstance();
        while (map.size() != 0) {
            components.map.Map.Pair<String, Integer> m = map.removeAny();
            String name = m.key();
            int salary = m.value();
            if (name.charAt(0) == initial) {
                salary += (raisePercent * salary / 100);
            }
            tmp.add(name, salary);
        }
        map.transferFrom(tmp);
    }
    ...
    public static void giveRaise(java.util.Map<String, Integer> map,
        ...
        while (it.hasNext()) {
            ...
            if (name.charAt(0) == initial) {
                int oldSalary = e.getValue();
                int newSalary = (raisePercent * oldSalary / 100) + oldSalary;
                e.setValue(newSalary);
            }
        }
    }
    ...
    public static void incrementAll(components.set.Set<NaturalNumber> set) {
        ...
        components.set.Set<NaturalNumber> tmp = set.newInstance();
        while (set.size() != 0) {
            NaturalNumber n = set.removeAny();
            n.increment();
            tmp.add(n);
        }
        set.transferFrom(tmp);
    }
    ...
    public static void incrementAll(java.util.Set<NaturalNumber> set) {
        ...
        java.util.Set<NaturalNumber> tmp = new HashSet<NaturalNumber>();
        while (it.hasNext()) {
